...
status.col <- "status"
sig.met <- met[rownames(dmc)[grep("hypo|hyper",dmc$status,ignore.case = TRUE)],]
...
sequence <- RangedData(space = as.character(seqnames(probes)),
                       IRanges(start = ranges(probes) %>% as.data.frame() %>% pull("start") - 100,
                               end = ranges(probes) %>% as.data.frame() %>% pull("end") + 100), strand = "*")
#look for motifs
