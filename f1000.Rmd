...
png("TF.png",width = 800, height = 200)
...
Due to time and memory constraints, we will use in this example only data from 11 LGG patients  and 11 GBM patients,
this patients should have both DNA methylation data and gene expression data.
...
matched_met_exp <- function(tumor, n = NULL){
    ...
    patients <- unique(substr(rnaseq.tp,1,12)[substr(rnaseq.tp,1,12) %in% substr(met450k.tp,1,12)] )
    if(!is.null(n)) patients <- patients[1:n] # get only n samples
    return(patients)
}
lgg.patients <- matched_met_exp("LGG", n = 10)
gbm.patients <- matched_met_exp("GBM", n = 10)
patients <- c(lgg.patients,gbm.patients)
...
query.met <- TCGAquery(tumor = c("GBM","LGG"), platform = "HumanMethylation450", level = 3, samples = patients)
TCGAdownload(query.met, samples = patients)
met.elmer <- TCGAprepare(query.met,dir = ".", save = FALSE,samples = patients)
...
TCGAdownload(query.rna, samples = patients, type = "rsem.genes.normalized_results")
exp.elmer <- TCGAprepare(query.rna,dir = ".", ,type = "rsem.genes.normalized_results",
                         samples =  patients, save = FALSE)
...
mee@sample$TN[substr(mee@sample$ID,1,12) %in% lgg.patients] <- "Control"
save(mee,file = "mee.rda")
```

To make this example faster, you can download the mee object from here.
...
The analysis found 10 enriched motifs for the hypo direction.


```{r tableTF, echo=FALSE, message=FALSE, warnings=FALSE, results='asis'}
library(knitr)
tabl <- kable(TF,format = "markdown")
cat(tabl) 
```
...
TFs that are associated with that AP1 motif are  WT1, ZNF208, ATF4 and DDX5.
