...
for(cancer in PanCancer){
    ...
    lsSample <- TCGAquery_samplesfilter(query = datQuery)
    dataSmTP <- TCGAquery_SampleTypes(barcode = lsSample$IlluminaHiSeq_RNASeqV2, typesample = "TP")[1:10]
...
for(tumor in PanCancer){
	...
	# get only primary solid Tumor
	lsSample <- TCGAquery_samplesfilter(query = datQuery)
	# For speed process we will use only 10 samples from LGG and 10 for GBM
	dataSmTP <- TCGAquery_SampleTypes(barcode = lsSample$IlluminaHiSeq_RNASeqV2, typesample = "TP")[1:10]
