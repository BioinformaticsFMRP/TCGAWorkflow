...
for(cancer in PanCancer){
    ...
    samples <- get(paste0(tolower(cancer),".samples"))
...
TCGAvisualize_meanMethylation(met,
                              groupCol = "disease", width = 6, height = 6,
                              group.legend  = "Groups",
...
for(tumor in PanCancer){
    ...
    selected <- TCGAquery_SampleTypes(barcode = lsSample$Genome_Wide_SNP_6, typesample = "TP")[1:100]
    ...
    dataAssay <- TCGAprepare(query = datQuery,
                             ...
                             summarizedExperiment = TRUE,
                             samples = selected)
    save(PlatformCancer, tumor, pathCancer, selected, dataAssay, file = paste0(tumor,"_",PlatformCancer,".rda"))
}
```
...
for(cancer in PanCancer){
    ...
    # For speed process we will evaluate only chromessome 1, chromosomes = -1 to consider all chromossomes
    results <- runGAIA(cnv_obj, markers_obj, output_file_name=paste0("GAIA_",tumor,"_",PlatformCancer,"_flt.txt"),
                       aberrations = -1,  chromosomes = c(14:18), num_iterations = 5, threshold = 0.25)
...
library(circlize)
png("CircosPlot.png",width=800,height=600)
...
png("CircosPlotChr17.png",width=800,height=600)
...
legend(0.25, 0.2, bty="n", y.intersp=1, c("Amp","Del"), pch=15, col=c("firebrick","forestgreen"),
       title="CNVs", text.font=3, cex=1.3, title.adj=0)
legend(0, 0.2, bty="n", y.intersp=1, names(colors), pch=16, col=colors,
       title="Mutations", text.font=3, cex=1.3, title.adj=0)
dev.off()
```
...
plot(analysis.jaspar, ncol=2, top=5, rev=FALSE, main="", bysim=TRUE, cex=0.7)
...
The make a motif enrichment plot you can sue the function motif.enrichment.plot
```{r, echo=TRUE, message=FALSE, warnings=FALSE, results='asis'}
