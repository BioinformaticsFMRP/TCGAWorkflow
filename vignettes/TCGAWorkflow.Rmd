...
```{r results='asis', echo=TRUE, message=FALSE,warning=FALSE,include=TRUE}
library(TCGAbiolinks)
...
nbsamples <- length(query.gbm.nocnv$results[[1]]$cases)
...
```{r results='asis', message=FALSE, warning=FALSE, include=FALSE}
...
sig.met <- met[values(met)[,"status.Glioblastoma.Multiforme.Brain.Lower.Grade.Glioma"] %in% c("Hypermethylated","Hypomethylated"),]
...
probes <- rowRanges(met)[values(met)[,"status.Glioblastoma.Multiforme.Brain.Lower.Grade.Glioma"] %in% c("Hypermethylated" ,"Hypomethylated") ,]
...
# Create a GR object based on the hypo/hypermethylated probes.
probes <- keepStandardChromosomes(rowRanges(met.chr9)[values(met.chr9)[,"status.Glioblastoma.Multiforme.Brain.Lower.Grade.Glioma"] %in% c("Hypermethylated","Hypomethylated"),])
# Defining a window of 3kbp - 3kbp_probe_3kbp
probes@ranges <- IRanges(start = c(probes@ranges@start - 3000), end = c(probes@ranges@start + 3000))
...
met.chr9 <- subset(met,subset = as.character(GenomicRanges::seqnames(met)) %in% c("chr9"))
met.elmer <- TCGAprepare_elmer(met.chr9, platform = "HumanMethylation450")
