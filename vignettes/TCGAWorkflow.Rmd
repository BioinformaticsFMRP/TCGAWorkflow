...
[ELMER](http://bioconductor.org/packages/ELMER/) steps for both the hypo
 direction, which means where are searching for distal enhancer probes hypomethylated in the GBM group. The
>>>>>>> upstream/master
...
=======
genes and the closest 2 downstream genes). The *mode* is set to "supervised" which will set group 1 (GBM samples)
as the U group, and the group 2 (LGG samples) as M group.
>>>>>>> upstream/master
...
set that does not contain a motif occurrence; $c$ and $d$ are the same counts within 
...
of the 20\% of samples with the highest average methylation at all motif-adjacent
probes, and the U group, which consisted of the 20\%  of samples with the lowest 
methylation. This step is performed by the *get.TFs* function, which takes *minSubgroupFrac* as an input parameter, 
again with a default of 20\%.
For each candidate motif-TF pair, the Mann-Whitney U test is used to test 
the null hypothesis that overall gene expression in group M is greater or equal 
than that in group U. This non-parametric test was used in order to minimize the 
effects of expression outliers, which can occur across a very wide dynamic range. 
...
=======
GBM group compared to the LGG group) with lower OR >= 1.3 is found in the Figure
>>>>>>> upstream/master
below. 
