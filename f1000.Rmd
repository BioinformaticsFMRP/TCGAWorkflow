...
exp.lgg <- GDCprepare(query.exp.lgg, save = FALSE)
...
exp.gbm <-GDCprepare(query.exp.gbm, save = FALSE)
exp.elmer <- SummarizedExperiment::cbind(exp.lgg, exp.gbm)
exp.elmer <- TCGAprepare_elmer(exp.elmer, platform = "IlluminaHiSeq_RNASeqV2")

LGGmut <- GDCquery_Maf(tumor = "LGG")
GBMmut <- GDCquery_Maf(tumor = "GBM")
...
rse <- get(load("GBMIllumina_HiSeq.rda"))
...
sig.met <- met.chr9[values(met.chr9)[,"status.Glioblastoma.Multiforme.Brain.Lower.Grade.Glioma"] %in% c("Hypermethylated","Hypomethylated"),]
...
ra = rowAnnotation(df = values(sig.met.100)["status.Glioblastoma.Multiforme.Brain.Lower.Grade.Glioma"],
                   col = list("status.Glioblastoma.Multiforme.Brain.Lower.Grade.Glioma" = c("Hypomethylated" = "orange",
...
# Save to png
png("heatmap.png",width = 600, height = 400); draw(heatmap, annotation_legend_side =  "bottom"); dev.off()
...
mee <- fetch.mee(meth = met.elmer, exp = exp.elmer, 
                 TCGA = TRUE, probeInfo = probe, geneInfo = geneInfo)
...
scatter.plot(mee, category="TN", save=T, lm_line=TRUE,
             byTF=list(TF=c("FOXD3","HIST1H1D","HIST1H2BH","HMGA2"), 
             probe=enriched.motif[["FOX"]]))
dev.off()
...
gbm_clin <- GDCquery_clinic("TCGA-GBM","Clinical")

# get clinical patient data for LGG samples
lgg_clin <- GDCquery_clinic("TCGA-LGG","Clinical")
...
TCGAsurvival_TFplot("ELF4",mee,clinical)
TCGAsurvival_TFplot("HMGA2",mee,clinical)
TCGAsurvival_TFplot("FOXD3",mee,clinical)
detach("package:ELMER", unload=TRUE)
R.utils::gcDLLs(quiet=F)
```
...
Biotechnological advances in sequencing have led to an explosion of
    ...
    This workflow introduces the following Bioconductor
    packages:
...
query <- GDCquery("TCGA-ACC",
                  ...
                  sample.type = c("Primary solid Tumor")) # see the barcodes with query$results[[1]]$cases
GDCDownload(query)
data <- GDCPrepare(query)
```
...
used as described in listing \[lst:clin\]. This function has two
...
query.clin <- GDCquery(project = "TCGA-GBM",
                   data.category = "Clinical",
                   barcode = c("TCGA-08-0516","TCGA-02-0317")) 
GDCDownload(query.clin)
clinical.patient <- GDCPrepare_clinic(query,"patient")
...
clinical.followup <- GDCPrepare_clinic(query,"follow_up")
clinical.nte <- GDCPrepare_clinic(query,"new_tumor_event")
clinical.stage <- GDCPrepare_clinic(query,"stage_event")
```
...
```{#numCode .R .numberLines}
mutation <- GDCquery_Maf(tumor = "ACC")
```
...
to identify recurrent alterations in cancer genome. We analyzed GBM and
LGG segmented CNV from SNP array (Affymetrix Genome-Wide Human SNP Array
6.0).

### Pre-Processing Data {#pre-processing-data .unnumbered}

The only CNV platform available for both LGG and GBM in TCGA is
“Affymetrix Genome-Wide Human SNP Array 6.0”. Using TCGAbiolinks, we
queried for CNV SNP6 level 3 data for primary solid tumor samples in the
legacy database. Data for selected samples were downloaded and prepared
in two separate rse objects (RangedSummarizedExperiment).
...
query.lgg.nocnv <- GDCquery(project = "TCGA-LGG",
                            ...
                            sample.type = c("Primary solid Tumor"))
result = tryCatch({
    GDCdownload(query.lgg.nocnv, chunks.per.download = 100)
}, error = function(e) {
    GDCdownload(query.lgg.nocnv, method = "client")
})
...
result = tryCatch({
    ...
    GDCdownload(query.gbm.nocnv, method = "client")
})
...
Recurrent amplifications and deletions were identified for both LGG
(Figure \[fig::cnvlgg\]) and GBM (Figure \[fig::cnvgbm\]), and
...
full length genes. An example of the result is shown in table
\[tab::tab1\].
...
```
  ...
  1   EIF4E2P1     Del          5.74967741935484e-05   20:20540891-21005246   20:20659710-20659964
  2   LLPHP1       Del          5.74967741935484e-05   20:20540891-21005246   20:20721187-20721879
  3   RN7SL607P    Del          5.74967741935484e-05   20:20540891-21005246   20:20738433-20738731
  4   MRPS11P1     Del          5.74967741935484e-05   20:20540891-21005246   20:20854121-20854642
  5   RPL24P2      Del          5.74967741935484e-05   20:21091497-21220212   20:21114723-21115197
  --- ------------ ------------ ---------------------- ---------------------- ----------------------

  : Chromosome 20 recurrent deleted genes in LGG<span
  data-label="tab::tab1"></span>
...
[@ComplexHeatmap]. The listing \[lst:mut\] shows how to download
...
![\[oncoprint\]Oncoprint for LGG samples. Blue defines SNP, green
defines insertions and red defines deletions. The upper barplot shows
the number of these genetic mutations for each patient, while the right
barplot shows the number of genetic mutations for each gene. The bottom
bar shows the group of each sample.
](figures/LGG_GBM_oncoprint.pdf){width="100.00000%"}
...
\[lst:dea\]). In this way we defined a square symmetric matrix of
...
\[eabarplot\] with the number of genes for the main categories of three
ontologies (i.e., GO:biological process, GO:cellular component, and
GO:molecular function).

The Figure \[eabarplot\] shows canonical pathways significantly
...
![\[eabarplot\] The plot shows canonical pathways significantly
overrepresented (enriched) by the DEGs (differentially expressed genes)
with the number of genes for the main categories of three ontologies
(GO:biological process, GO:cellular component, and GO:molecular
function, respectively). The most statistically significant canonical
pathways identified in DEGs are listed according to their p-value
corrected FDR (-Log10) (colored bars) and the ratio of listed genes
found in each pathway over the total number of genes in that pathway
(ratio, red
line).](figures/TCGAvisualize_EAbarplot_Output.png){width="100.00000%"}
...
[@luo2013pathview] can be used. Listing \[lst:pathview\] shows an
...
(’hsa’ for Homo sapiens) and the limits for the gene expression (see
figure \[gliomapathview\]).
...
![\[gliomapathview\] Pathways enrichment analysis : glioma pathway. Red
defines genes that are up-regulated and green defines genes that are
down-regulated.](figures/hsa05214_pathview.png){width="100.00000%"}
...
We start this analysis by inferring two gene regulatory networks (the
corresponding number of edges are presented in
Table \[table::numberedges\]) for the GBM data set and one gene set for
the LGG data.
...
\[t!\] ![image](figures/roc_biogrid_GBM.png){width="100.00000%"}

In Figure \[fig::roc\_GBM\], the obtained ROC curve and the
...
(lines 1-56 of the listing \[lst:meanmet\] describes how to make the
...
Therefore, the rs probes were removed (see listing \[lst:meanmet\] lines
...
(see listing \[lst:meanmet\] lines 65).
...
(accessed by the *colData* function) (see listing \[lst:meanmet\] lines
70-74).
...
Figure \[mean\_met\] illustrates a mean DNA methylation plot for each
...
difference between the groups of tumors (p-value = $6.1x10^{-06}$).

![\[mean\_met\] Boxplot of mean DNA methylation of each sample (black
dots)](figures/mean_lgg_gbm.png){width="50.00000%"}
...
(see listing \[lst:dmrcode\]). The DNA methylation data (level 3) is
...
difference of 0.25 and an adjusted p-value of less than $10^{-2}$.
...
with the results in the rowRanges. Figure \[volcano\_met\] shows the
volcano plot produced by listing  \[lst:dmrcode\]. This plot aids the
...
![\[volcano\_met\] Volcano plot: searching for differentially methylated
CpG sites (x-axis:difference of mean DNA methylation, y-axis:
statistical
significance)](figures/LGG_GBM_metvolcano.png){width="100.00000%"}
...
additional metadata description. The listing \[lst:metheatmap\] shows
the code to produce the heatmap of a DNA methylation datum (Figure
\[complexheatmap\_met\]).

![\[complexheatmap\_met\] Heatmap of DNA methylation in probes. Rows are
probes and columns are samples (patients). The DNA methylation values
range from 0.0 (completely DNA unmethylated, blue) to 1.0 (completely
DNA methylated, red). The groups of each sample were annotated in the
top bar and the DNA methylation status for each probe was annotated in
the right bar](figures/heatmap.pdf){width="100.00000%"}
...
in the listing \[lst:rgadem\]). An object will be returned which
...
of multiple motifs with different similarity scores (see Figure
\[fig::motif\]).
```{r results='asis', echo=TRUE, message=FALSE,warning=FALSE}
library(rGADEM)
...
probes <- rowRanges(met.chr9)[values(met.chr9)[,"status.Glioblastoma.Multiforme.Brain.Lower.Grade.Glioma"] %in% c("Hypermethylated" ,"Hypomethylated") ,]
...
![Motif logos found during *de-novo* motif analysis<span
data-label="fig::motif"></span>](figures/gadem.png){width="60.00000%"}
...
to start the motif matching analysis (line 4 of listing \[lst:MotIV\]).
The result is shown in Figure \[fig::tfmotif\].
...
value of $10^{-2}$ for the DNA methylation data. The starburst plot for
the listing \[lst:starburst\] is shown in Figure \[starplot\]. While the
...
that respects these parameters (circled genes in Figure \[starplot\]).
...
is shown in table \[histone-mark\].
...
\[lst:annotationhub\] is to download the chip-seq data. The function
...
After downloading the histone marks (see listing \[lst:annotationhub\],
...
understand better the regions found. Listing \[lst:annotationhub\_3\]
shows an example of code to plot the average profile. Figure
\[chip-seq\] shows the result.
...
the AnnotationHub package that can access the Roadmap database(listing
\[lst:annotationhub\]). Next, the Chipseeker was used to visualize how
histone modifications are enriched onto hypomethylated and
hypermethylated regions, (listing \[lst:annotationhub\_3\]). The
enrichment heatmap and the average profile of peaks binding to those
region is shown in Figures \[chip-seq-heatmap\] and \[chip-seq\]
respectively.
...
probes <- GenomeInfoDb::keepStandardChromosomes(rowRanges(met.chr9)[values(met.chr9)[,"status.Glioblastoma.Multiforme.Brain.Lower.Grade.Glioma"] %in% c("Hypermethylated","Hypomethylated"),])
...
The listing \[lst:prepareelmer\] shows how to use
[TCGAbiolinks](http://bioconductor.org/packages/TCGAbiolinks/)
...
GDCdownload(query.gbm)
met.gbm <- GDCprepare(query.gbm, save = FALSE)
met.elmer <- SummarizedExperiment::cbind(met.lgg, met.gbm)
met.elmer <- TCGAprepare_elmer(met.elmer, 
...
for (j in direction){
    ...
    # Step 3.1: Get diff methylated probes |
    #--------------------------------------
    Sig.probes <- get.diff.meth(mee, cores=parallel::detectCores(),
                                dir.out =dir.out,
                                diff.dir=j,
                                pvalue = 0.01)
    ...
    if(length(Sig.probes.paired) > 0 ){
        ...
        if(length(enriched.motif) > 0){
            ...
            TF <- get.TFs(mee = mee,
                          enriched.motif = enriched.motif,
                          dir.out = dir.out,
                          cores = parallel::detectCores(), label = j)
            TF.meth.cor <- get(load(paste0(dir.out,
	                                   "/getTF.",j,
					   ".TFs.with.motif.pvalue.rda")))
            save(TF, enriched.motif, Sig.probes.paired,
...
motif. We can see in Figure \[tfrank\] that the top three TFs that are
associated with that FOX motif are FOXD3, HMGA2 and HIST1H2BH.
...
For example, Figure \[tfvsmet\] shows the average DNA methylation level
of sites with the FOX motif plotted against the expression of the TFs
FOXD3, HIST1H1D, HIST1H2BH and HMGA2. We can see that the experiment
group (LGG samples) has a lower average methylation level of sites with
the FOX motif plotted and a higher average expression of the TFs.
```{#numCode .R .numberLines}
scatter.plot(mee, category="TN", save=T, lm_line=TRUE,
...
And for each relevant TF we will use the clinical data to analyze the
survival curves for the 30% patients with higher expression of that TF
versus the 30% with lower expression. The code below shows that
analysis.
...
The Figure \[survival\_hoxa6\], shows that the samples with lower
expression of some of these TFs have a better survival than those with
higher expression.
...
[TCGAbiolinks](http://bioconductor.org/packages/TCGAbiolinks/), dnet,
...
Bioconductor project, running on R version 3.2.2 or higher. It requires
a number of software packages, including
[AnnotationHub](http://bioconductor.org/packages/AnnotationHub/),
[ChIPSeeker](http://bioconductor.org/packages/ChIPseeker/),
[ELMER](http://bioconductor.org/packages/ELMER/),
[ComplexHeatmap](http://bioconductor.org/packages/ComplexHeatmap/),
[GAIA](http://bioconductor.org/packages/gaia/),
[rGADEM](http://bioconductor.org/packages/rGADEM/),
[MotIV](http://bioconductor.org/packages/MotIV/),
[MINET](http://bioconductor.org/packages/minet/),
[RTCGAToolbox](http://bioconductor.org/packages/RTCGAToolbox/) and
[TCGAbiolinks](http://bioconductor.org/packages/TCGAbiolinks/). Version
numbers for all packages used are in section “Session Information”.
Listing \[lst:install\] shows how to install all the required packages.
...
```
All data used in this workflow is freely available and some of them can
be accessed using a R/Bioconductor package. There are three main sources
of data: [The NCI Genomic Data Commons
(GDC)](http://cancergenome.nih.gov/), a [supplementary data
repository](http://egg2.wustl.edu/roadmap/web_portal/) with processed
datasets from the Roadmap Epigenomics Project and from The Encyclopedia
of DNA Elements (ENCODE) [@kundaje2015integrative] and the
[BioGRID](http://www.thebiogrid.org) database with physical and genetic
interactions information. For the first, a summary of the data available
can be seen in <https://gdc-portal.nci.nih.gov/> and its data can be
accessed using the R/Bioconductor
[TCGAbiolinks](http://bioconductor.org/packages/TCGAbiolinks/) package.
For the second, a summary of the data available can be found in this
[spread
sheet](https://docs.google.com/spreadsheets/d/1yikGx4MsO9Ei36b64yOy9Vb6oPC5IBGlFbYEt-N6gOM/edit#gid=15)
and the data can be accessed using the R/Bioconductor
[AnnotationHub](http://bioconductor.org/packages/AnnotationHub/)
package. For the third, the data should be directly download from the
data portal.
