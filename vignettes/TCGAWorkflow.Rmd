...
# This data is avaliable in the package in the elmerExample 
```

```{r echo=TRUE, message=FALSE,warning=FALSE}
data(elmerExample, package = "TCGAWorkflowData")
...
met <- TCGAanalyze_DMR(met,
                       ...
                       cores =  parallel::detectCores()/2 # if set to 1 there will be a progress bar
)
```
...
Recent studies indicate that DNA methylation can be used to identify changes at 
transcriptional enhancers and other cis-regulatory elements in primary human samples. 
Many studies suggest that enhancers play a major role as
...
A systematic approach to inferring gene regulatory networks has been provided by the 
R/Bioconductor package [Enhancer Linking by Methylation/Expression Relationship
(ELMER)](http://bioconductor.org/packages/ELMER/), which first identifies DNA methylation changes in 
distal regulatory elements and correlates these with expression of nearby genes to identify direct transcriptional targets.
Next, ELMER performs a transcription factor binding motif analysis and integrates with expression profiling of all human transcription factors, to identify master regulatory TFs and place each differentially methylated regulatory element into the context of an altered gene regulatory network (GRN). This package can be easily applied to TCGA public available
cancer data sets and custom DNA methylation and gene expression data
sets [@yao2015inferring; @ELMER2].
...
This section shows how to use ELMER (version 2) to analyze TCGA data using as
...
MAE (MutiAssayExperiment) object that contains a DNA methylation Summarized Experiment object, an gene expression
 Summarized Experiment object, a data frame mapping for each sample its DNA methylation data and its gene expression data,
and a metadata data frame. It should be highlighted that samples
without both the gene expression and DNA methylation data are
removed from the MAE object. This object can be created using the function *createMAE*.
In this function we use `distal.probes` object is to filter the DNA methylation probes used in the analysis which are
 probes at least 2kb from the TSS,and we set `linearize.exp` to TRUE to take the log2(expression + 1) of the expression matrix in order to linearize the relation between DNA methylation and expression.
...
[ELMER](http://bioconductor.org/packages/ELMER/) steps for the 
distal probes hypermethylated in the LGG group direction.
Some important ELMER arguments are `minSubgroupFrac`,`diff.dir` and `mode`.
`minSubgroupFrac` controls the percentage of samples used in the analysis, in
our example we will use all samples. `diff.dir` is used in the first step to identify the 
probes differently methylated in group1 vs group2, if set to "hypo" it will select the hypomethylated
probes in group 1, if set to "hyper" it will select the hypermethylated ones. The `mode` argument can be set to
"supervised" or "unsupervised" and they control how the groups of Methylated and Methylated Samples are selected 
in steps 3 and 5. If it is set to "supervised", if `diff.dir` is set to "hypo" Unmethylated group will be the group 1 and the Methylated will be the group 2 (if `diff.dir` is set to "hyper" the groups are inversed), but if it is set to "supervised"
the algorithm will actually put all the data togther sort by DNA methylation level and the split between Unmethylated and Methylated group
there might be cases in which samples of both group1 and group2 are in one of the Unmethylated and Methylated groups.
It is advised to use "supervised" mode if you are dealing with known molecular subtypes, if not, the `mode` should be set to "unsupervised" [@ELMER2].
An example of analysis using the "unsupervised" would be Tumor vs Normal, while the  "supervised" would be molecular subtype 1 vs molecular subtype 2.
In our example we will set our mode to "supervised".
...
When ELMER Identifies the enriched motifs for the distal probes
...
The list of motifs for the  hypermethylated probes in the LGG group is found in the Figure below. 

```{r, echo=TRUE, message=FALSE, warnings=FALSE, results='asis',fig.height=4}
motif.enrichment.plot(motif.enrichment = motif.enrichment, save = FALSE,significant = list(lowerOR = 1.1, NumOfProbes = 10))
```
...
associated with a motif found. Also, using the TFClass database,
which classifies TF in to families and subfamilies, for a given motif,
the TF within the same family of the motif is highlighted.
...
2.  top.potential.TF.family: the highest ranking upstream TFs which are known
recognized the motif (considering the TFClass family classificaiton).

3.  potential.TFs.family: TFs which are within top 5% list and are known
recognized the motif (considering the TFClass family classificaiton).

4.  top.potential.TF.subfamily: the highest ranking upstream TFs which are known
recognized the motif (considering the TFClass subfamily classificaiton). 

5.  potential.TFs.family: TFs which are within top 5% list and are known
recognized the motif (considering the TFClass subfamily classificaiton).

6. top5percent: all TFs which are within top 5%
