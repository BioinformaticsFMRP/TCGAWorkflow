...
```
```{r, echo=FALSE, message=FALSE, warnings=FALSE, results='asis',fig.height=10}
motif.enrichment.plot(motif.enrichment=motif.enrichment,save = FALSE)
```
...
library(ELMER)
library(grid)
TF.rank.plot(motif.pvalue=TF.meth.cor, motif="AP1", save=FALSE)
```	
...
scatter.plot(mee, category="TN", save=T, lm_line=TRUE,
             byTF=list(TF=unlist(str_split(TF[1,"top_5percent"],";"))[1:4], 
             probe=enriched.motif[[TF$motif[1]]]))
dev.off()
```
```{r results='asis', echo=FALSE, message=FALSE, warning=FALSE}
png("TF.png",width = 800, height = 400)
scatter.plot(mee, category="TN", save=F, lm_line=TRUE,
             byTF=list(TF=unlist(str_split(TF[1,"top_5percent"],";"))[1:4], 
             probe=enriched.motif[[TF$motif[1]]]))
dev.off()
```
...
TCGAsurvival_TFplot(unlist(str_split(TF[1,"top_5percent"],";"))[1],mee,clinical)
TCGAsurvival_TFplot(unlist(str_split(TF[1,"top_5percent"],";"))[2],mee,clinical)
TCGAsurvival_TFplot(unlist(str_split(TF[2,"top_5percent"],";"))[1],mee,clinical)
```
