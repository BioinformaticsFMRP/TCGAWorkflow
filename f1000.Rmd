...
\begin{figure}[ht!]
\centering
\includegraphics[width=0.5\textwidth]{figures/mean_lgg_gbm.png}
\caption{\label{mean_met} Boxplot of  mean DNA methylation of each sample (black dots)}
\end{figure}

\subsubsection*{Searching for differentially methylated CpG sites}
...
\begin{lstlisting}[language=R,caption={Finding differentially
methylated CpG sites},label=lst:dmrcode]
...
The bioconductor package \href{http://bioconductor.org/packages/rGADEM/}{rGADEM} \cite{droit2015rgadem,li2009gadem} provides an efficient \textit{de novo} motif discovery algorithm for large-scale genomic sequence data.
...
Using bioconductor package \href{http://bioconductor.org/packages/motifStack/}{motifStack} \cite{ou2013motifstack} it is possible to generate a graphic representation of multiple motifs with different similarity scores (see figure \ref{fig::motif}).
...
After \href{http://bioconductor.org/packages/rGADEM/}{rGADEM} returns it's results, the user can use \href{http://bioconductor.org/packages/MotIV/}{MotIV}
package \cite{mercier2014motiv,mahony2007dna,mahony2007stamp,mercier2011integrated}
...
Recent studies have shown that providing a deep integrative analysis can aid researchers in identifying and extracting biological insight from high through put data \cite{phillips2008role,shi2014integrative,rhodes2005integrative}.
In this section, we will introduce a bioconductor package called \href{http://bioconductor.org/packages/ELMER/}{ELMER} to identify regulatory enhancers using gene expression + DNA methylation data + motif analysis.  In addition, we show how to integrate the results from the previous sections with important epigenomic data derived from both the \href{http://www.encodeproject.org/}{ENCODE} and \href{http://www.roadmapepigenomics.org/}{Roadmap}.
...
probes <- keepStandardChromosomes(rowRanges(met)[values(met)$status.GBM.LGG %in% c("Hypermethylated","Hypomethylated"),])
