...
4. Identification of enriched motifs within set of probes in significant probe-gene pairs.
...
After preparing the data into a MAE object, we executed the five
...
are identified is found in the [ELMER.data](https://bioconductor.org/packages/release/data/experiment/vignettes/ELMER.data/inst/doc/vignettes.pdf) vignette. As the numer of samples are small we loosened some of the cut-off values.
...
### Identification of distal probes with significant
...
Also, to make the example faster, the numner of permutation is reduced to 5 (*permu.size* = 5), the raw p-value cut-off is loosened to 0.1 and the empirical p-value to 0.5, but it is recommended in a real analysis to use the default values.
